      ...
      if (response.statusCode === 200) {
        ...
        if (user) {
          ...
          localStorage.setItem(
            "user",
            JSON.stringify({ name: user.name, lastName: user.lastName, email: user.email })
          );
          dispatch(
            setUser({ name: user.name, lastName: user.lastName, email: user.email, id: user.id })
          );
        }
