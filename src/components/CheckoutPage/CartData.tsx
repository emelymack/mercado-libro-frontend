...
const CartData = () => {
  ...
  return (
    <Box py={5} px={6} border={'2px solid'} borderColor={'brand.greenLogo'} borderRadius={10} mt={{base: 5, lg: 0}}>
      {cartData.items.map((item) => { console.log(item)
        return (
        <Flex alignItems={"center"} justifyContent={"space-between"} mb={4}>
          ...
          <Text color={'brand.blueLogo'} fontWeight={600} fontSize={'lg'} mt={2}>{formatPrice(item.product.price)}</Text>
        </Flex>
      )})}
      <Divider mt={6} />
      <Box>
        ...
        <Flex alignItems={"center"} justifyContent={"space-between"} my={4}>
          ...
          <Text color={'brand.blueLogo'} fontWeight={500}>{cartData.shipping.type === 'ENVIO_DOMICILIO' ? formatPrice(cartData.shipping.price) : 'Gratis'}</Text>
        </Flex>
        <Flex alignItems={"center"} justifyContent={"space-between"} my={4}>
